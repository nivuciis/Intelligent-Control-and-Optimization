...
from simulation import setup_motor, run_simulation_cost, connected_systems_model
from utils import get_ref

def plot_results(method_name, initial_pid, optimized_pid):
    print(f"\n[Plotting] Comparando PID Inicial vs {method_name}...")
    t_eval = np.linspace(0, TF, 1000)
    
    # Without Optimization Simulation
    ...
    plt.plot(t_eval, get_ref(t_eval, "ramp"), 'k--', label='Referência')
    ...
    plt.title(f"Otimização via {method_name}")
    plt.xlabel('Tempo (s)')
    plt.ylabel('Torque')
    plt.legend()
    plt.grid(True)
